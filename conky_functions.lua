...
function conky_reload(event)
    ...
    if event.x >= button_x and event.x <= button_x + button_w and
       event.y >= button_y and event.y <= button_y + button_h then
            ...
            -- os.execute("pkill conky && conky -b &")
    end
end
...
require 'cairo'
-- Function to handle mouse clicks
function conky_weather_reload(event)
    local button_x, button_y, button_w, button_h = 1, 95, 20, 20
...
function conky_draw_reset_weather()
    if conky_window == nil then return end
    local cs = cairo_xlib_surface_create(conky_window.display, conky_window.drawable, conky_window.visual, conky_window.width, conky_window.height)
    local cr = cairo_create(cs)

    -- Button dimensions and position
    local x, y, width, height = 1, 95, 10, 10
